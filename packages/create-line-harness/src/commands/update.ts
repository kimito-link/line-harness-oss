...
export async function runUpdate(repoDir: string): Promise<void> {
  ...
  // Ensure cookie-based admin auth is configured. Installs created before this
  // version never set ADMIN_ORIGIN / ADMIN_ALLOW_CROSS_SITE, so the new
  // cross-site cookie auth would break their admin login on upgrade. Derive the
  // admin origin from saved state (or the deployed Pages project) and set the
  // bindings here so upgrades match a fresh setup.
  const adminUrl = (savedState?.adminUrl as string | undefined)
    ?? `https://${adminProjectName}.pages.dev`;
  await configureAdminAuth({
    workerName: projectName,
    workerUrl: savedState?.workerUrl as string | undefined,
    adminUrl,
  });
